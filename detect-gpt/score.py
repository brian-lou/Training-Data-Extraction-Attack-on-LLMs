...
def get_perturbation_results(span_length=10, n_perturbations=1, n_samples=500):
    ...
    for _ in range(n_perturbation_rounds - 1):
        try:
            ...
            break
    p_sampled_text = p_original_text
    ...
    for res in tqdm.tqdm(results, desc="Computing log likelihoods"):
        ...
        p_sampled_ll = p_original_ll
        res["original_ll"] = get_ll(res["original"])
        res["sampled_ll"] = res["original_ll"]
        res["all_perturbed_original_ll"] = p_original_ll
        res["all_perturbed_sampled_ll"] = res["all_perturbed_original_ll"]
        res["perturbed_original_ll"] = np.mean(p_original_ll)
        res["perturbed_sampled_ll"] = res["perturbed_original_ll"]
        res["perturbed_original_ll_std"] = np.std(p_original_ll) if len(p_original_ll) > 1 else 1
        res["perturbed_sampled_ll_std"] = res["perturbed_original_ll_std"] 
...
if __name__ == '__main__':
    ...
    SAVE_FOLDER = f"tmp_results/{output_subfolder}"
